...
bool WinInit(HINSTANCE hInstance, int nCmdShow, WNDCLASSEX *uwc, const char *title, bool fixedsize)
{
    ...
    if(uwc) {
        memcpy_s(&wndclass, sizeof(wndclass), uwc, sizeof(wndclass));
...
static bool loop_once(video *v)
{
    ...
    if(v->calc_fps && msec >= g_msec+1000) {
        ...
        char buffer[256], n = _snprintf_s(buffer, 256, 128, "%s: %d fps", v->title, int(double(g_fps + g_skips)/sec));
        if(g_skips) _snprintf_s(buffer+n, 256-n, 128, " - %d skipped = %d updates", int(g_skips/sec), int(g_fps/sec));
